...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('toysGames.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('toysGames.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('toysGames.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
